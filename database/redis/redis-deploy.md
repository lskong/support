...
# redis-cluster-proxy
node1 10.0.0.23 6380

# 每个节点启动两个redis（一主一从）,主从交叉部署

# 主从关系图：
#  主：23:6381  24:6381  25:6381
#         |         |       |
#  从：24:6382  25:6382  23:6382
```
...
```shell
# node1 
mkdir -p /data/redis-cluster/{data,logs,conf,pid}
mkdir -p /data/redis-cluster/{data,logs,conf,pid}
...
sudo curl https://packages.redis.io/gpg | sudo apt-key add -
sudo echo "deb https://packages.redis.io/deb $(lsb_release -cs) main" | sudo tee /etc/apt/sources.list.d/redis.list
sudo apt-get update
sudo apt-get install redis

for i in {1..3}
do
ssh node${i} "apt install -y redis;redis-cli --version"
done 
...
```shell
# 生成配置文件
cat > /data/redis-cluster/conf/6381.conf <<EOF
port 6381
requirepass 1234
masterauth 1234
protected-mode no
...
save ""
appendonly no
pidfile /data/redis-cluster/pid/6381.pid
logfile /data/redis-cluster/logs/6381.log
maxclients 10000
io-threads 8
...
disable-thp yes
cluster-enabled yes
cluster-config-file /data/redis-cluster/data/6381-run.conf
...
cat > /data/redis-cluster/conf/6382.conf <<EOF
port 6382
requirepass 1234
masterauth 1234
protected-mode no
...
save ""
appendonly no
pidfile /data/redis-cluster/pid/6382.pid
logfile /data/redis-cluster/logs/6382.log
maxclients 10000
io-threads 8
io-threads-do-reads yes
oom-score-adj no
disable-thp yes
cluster-enabled yes
cluster-config-file /data/redis-cluster/data/6382-run.conf
cluster-node-timeout 1500
cluster-announce-ip 10.0.0.23
...
# node2/node3修改配置文件
sed -i 's/10.0.0.23/10.0.0.24/g' /data/redis-cluster/conf/6381.conf
sed -i 's/10.0.0.23/10.0.0.24/g' /data/redis-cluster/conf/6382.conf

sed -i 's/10.0.0.23/10.0.0.25/g' /data/redis-cluster/conf/6381.conf
sed -i 's/10.0.0.23/10.0.0.25/g' /data/redis-cluster/conf/6382.conf
```
...
[Unit]
Description=redis cluster node 6381 daemon
After=network.target redis-6381.service
Wants=redis-6381.service

[Service]
Type=simple
ExecStart=/usr/bin/redis-server /data/redis-cluster/6381/conf/redis-6381.conf
ExecStop=/bin/kill -s TERM $MAINPID
PIDFile=/data/redis-cluster/6381/pid/redis-6381.pid
TimeoutStopSec=0
Restart=always
...
cp /usr/lib/systemd/system/redis-6381.service /usr/lib/systemd/system/redis-6382.service
sed -i 's/6381/6382/g' /usr/lib/systemd/system/redis-6382.service
...
```shell
# 创建集群
redis-cli -a 1234 --cluster create 10.0.0.23:6381 10.0.0.24:6381 10.0.0.25:6381 10.0.0.24:6382 10.0.0.25:6382 10.0.0.23:6382 --cluster-replicas 1 --cluster-yes
# 需要输入yes
