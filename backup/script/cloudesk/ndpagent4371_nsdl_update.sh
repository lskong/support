...
G_NDP_NSDL_VER="NDP_NSDL_V4.3.7.1"
...
ndp(){
    ...
    if [ -f ${G_NDP_NSDL_VER}.zip ];then
        unzip -oq ${G_NDP_NSDL_VER}.zip && rm -f ${G_NDP_NSDL_VER}.zip
...
mono(){
    ...
    [ -f ${G_MONO_SERVICE} ] && rm -f ${G_MONO_SERVICE}.*
...
removendp(){
    ...
    # kill NDP process
    systemctl disable nbp-mono.service >/dev/null 2>&1
